...
knitr::kable(summary_stats_tables, format = "markdown", width = "90%") 
```

In @tbl-summarystats, we have summarized the mean values for each of the measured values from the wine. This gives us a better idea of the normal range of values for each predictor within our model.
...
## Discussion
![Model accuracy as a function of number of neighbors](../results/accuracy_plot.png){#fig-accuracy-plot width=90%}
...
We can see from @fig-accuracy-plot that accuracy remains high across from k = 1 to k = 20. It peaks at around $`{r} accuracy_val` %$ for $K = 8$ before decreasing and subsequently increasing back to $`{r} accuracy_val` %$ from $K = 17$ to $K = 19$. This is a high accuracy value and will increase the power of our predictive model.
