...
test_that("`create_scatter_plot` should return a ggplot", {
  scatter_plot <- create_scatter_plot(wine_test_data, "alcohol", "total_phenols") # call the function
...
test_that("`create_scatter_plot` should throw an error when incorrect types 
are passed to `data`, `scatter1`, and `scatter2` arguments", {
  expect_error(create_scatter_plot(wine_data_fake, "alcohol", "total_phenols"), info = "Dataframe should be inputted")
  expect_error(create_scatter_plot(wine_test_data, 'variable1', "total_phenols"), info = "Valid x variable should be inputted")
  expect_error(create_scatter_plot(wine_test_data, 'alcohol', "variable2"), info = "Valid y variable should be inputted")
})

# test that the correct data is being plotted on the axes
test_that("`create_scatter_plot` correctly maps data to x and y axes", {
...
test_that("`create_scatter_plot` has the correct axis labels", {
  scatter_plot <- create_scatter_plot(wine_test_data, "alcohol", "total_phenols") # make the scatterplot
...
test_that("`create_scatter_plot` colors points with the same cultivar the same color", {
  scatter_plot <- create_scatter_plot(wine_test_data, "alcohol", "total_phenols") # create the scatterplot
...
test_that("`create_scatter_plot` assigns different colors to different cultivars", {
  scatter_plot <- create_scatter_plot(wine_test_data, "alcohol", "total_phenols") # create the scatterplot
